...
async def fetch_availabilty_of_doctor(doctor_id, limit: int = 10, page: int = 0, db: AsyncSession = Depends(get_db), current_user=Depends(auth.get_current_user)):
...
async def set_availabilty(request: availbilitySchema.AvailabilitySetRequestCO, db: AsyncSession = Depends(get_db), current_user=Depends(roles.require_role(UserRole.DOCTOR))):
